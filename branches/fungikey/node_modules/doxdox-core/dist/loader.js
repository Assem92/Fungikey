 ...
 * @public
 */
export const loadPluginFromPackagePath = async (path) => {
    try {
 ...
 * @public
 */
export const loadPluginFromFile = async (path) => {
    try {
 ...
 * @public
 */
export const loadPlugin = async (directory, prefix, pathOrPackage) => {
    const prefixPattern = new RegExp(`^${prefix || ''}`);
    try {
        if (await isFile(pathOrPackage)) {
            return await loadPluginFromFile(pathOrPackage);
        }
        else if (await isDirectory(join(directory, `${prefix}${pathOrPackage.replace(prefixPattern, '')}`))) {
            return await loadPluginFromPackagePath(join(directory, `${prefix}${pathOrPackage.replace(prefixPattern, '')}`));
        }
        else if (await isDirectory(join(directory, pathOrPackage.replace(prefixPattern, '')))) {
            return await loadPluginFromPackagePath(join(directory, pathOrPackage.replace(prefixPattern, '')));
        }
    }
    catch (err) {
        if (process.env.DEBUG) {
        ...
        throw new Error(`${prefix}${pathOrPackage.replace(prefixPattern, '')} plugin was not found!`);
    }
    return null;
};
