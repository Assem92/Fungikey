 ...
 * @public
 */
export const findFileInPath = async (input, fileName = 'package.json') => {
    try {
 ...
 * @public
 */
export const findParentNodeModules = async (currentDirectory, 
 ...
 * @public
 */
export const getProjectPackage = async (cwd) => {
    const projectPackagePath = await findFileInPath(cwd);
    if (projectPackagePath) {
        const { name, description, version, exports, homepage, doxdoxConfig } = JSON.parse(await fs.readFile(projectPackagePath, 'utf8'));
        return {
            name,
            description,
            version,
 ...
 * @public
 */
export const getRootDirPath = (url) => dirname(fileURLToPath(url || import.meta.url));
/**
 ...
 * @public
 */
export const parseConfigFromCLI = (rawFlags) => rawFlags
 ...
 * @public
 */
export const parseIgnoreConfig = (contents) => contents
    .trim()
    .split(EOL)
 ...
 * @public
 */
export const sanitizePath = (path) => new URL(path).pathname;
/**
 ...
 * @public
 */
export const slugify = (contents) => contents
    .toLowerCase()
    .replace(/[^a-z]+/g, '-')
    .replace(/^-|-$/, '');
