...
const md = new MarkdownIt({
    html: true,
    linkify: true,
    highlight: str => `<div class="bg-light p-3">${hljs.highlight(str, { language: 'javascript' }).value}</div>`
});
const mdTypes = new MarkdownIt();
const renderMethod = (method) => `<div class="mb-5" data-method-name="${method.name}"><a name="${method.slug}" />

<h2 class="method-name">
  <a href="#${method.slug}" class="method-permalink" aria-label="Permalink">#</a>
  ${method.fullName}
</h2>
...
${method.returns.map(param => `<p><code>${param.types
        .map(type => mdTypes.renderInline(type))
...
`;
const renderFileNav = (file) => `<p><a href="#${file.path}" class="file-name"><b>${file.path}</b></a></p>
<ul class="list-unstyled ml-0">
${file.methods
    .map(method => `<li class="method-name" data-method-name="${method.name}"><a href="#${method.slug}" class="${method.private ? 'text-muted' : ''}">${method.name}</a></li>`)
    .join('')}
</ul>`;
const renderFile = (file) => `<a name="${file.path}" />
${file.methods.map(method => renderMethod(method)).join('')}`;
export default async (doc) => `<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
