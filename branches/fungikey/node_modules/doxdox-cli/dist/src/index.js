...
import doxdox, { findFileInPath, findParentNodeModules, getProjectPackage, getRootDirPath, loadPlugin, parseConfigFromCLI, parseIgnoreConfig, sanitizePath } from 'doxdox-core';
...
const showVersion = Boolean(args.flags['-v'] || args.flags['--version'] || false);
const overrideName = String(args.flags['-n'] || args.flags['--name'] || '');
const overrideDescription = String(args.flags['-d'] || args.flags['--description'] || '');
const overrideIgnore = String(args.flags['-i'] || args.flags['--ignore'] || '');
const overrideParser = String(args.flags['-l'] || args.flags['--parser'] || 'jsdoc');
const overrideRenderer = String(args.flags['-r'] || args.flags['--renderer'] || 'markdown');
const overrideOutput = String(args.flags['-o'] || args.flags['--output'] || '');
const overridePackage = String(args.flags['-p'] || args.flags['--package'] || '');
(async () => {
    const pkgPath = await findFileInPath(join(getRootDirPath(import.meta.url), '..'));
    if (showHelp) {
    ...
    const paths = await globby((args.inputs.length ? args.inputs : defaultPaths).concat(parseIgnoreConfig(overrideIgnore.split(',').join(EOL))), {
        cwd,
        ignoreFiles: ['.doxdoxignore'],
        gitignore: true
    });
    const cliConfig = parseConfigFromCLI(args.raw);
    const nodeModulesDir = await findParentNodeModules(dirname(sanitizePath(import.meta.url)));
    if (!nodeModulesDir) {
        throw new Error('node_modules directory was not found');
    }
    const loadedParser = await loadPlugin(nodeModulesDir, 'doxdox-parser-', overrideParser.toLowerCase());
    const loadedRenderer = await loadPlugin(nodeModulesDir, 'doxdox-renderer-', overrideRenderer.toLowerCase());
    if (!loadedParser) {
