...
const parser = async (cwd, path) => {
    try {
        ...
        const output = await spawn(join(nodeModulesDir, `.bin/${platform() === 'win32' ? 'jsdoc.cmd' : 'jsdoc'}`), [
            '--explain',
            join(cwd, path),
            '--configure',
        ...
        const methods = docs
            .filter((jsdoc) => jsdoc.kind === 'function' && !jsdoc.undocumented)
            .map((jsdoc) => {
            const params = (jsdoc.params || []).map(({ name = null, description = null, type = {} }) => ({
                name,
                description,
                types: type.names || []
            }));
            const returns = (jsdoc.returns || []).map(({ name = null, description = null, type = {} }) => ({
                name,
                description,
                types: type.names || []
            }));
            const isPrivate = jsdoc.access === 'private' ||
                (jsdoc.tags &&
                    jsdoc.tags.findIndex(tag => tag.title === 'api' && tag.value === 'private') !== -1) ||
                false;
            return {
...
export const parseString = async (path, content) => {
    temp.track();
